    ...
    private static void PopulateDataFromBlueprint(HousePlanSO plan)
{
    ...
    // MB South Wall is at Z = mbPos.z. Vestibule is mvDepth deep (N-S).
    // MB is mbWidth wide. Vestibule mvWidth wide. Place SW corner of Vestibule.
    Vector3 mvPos = new Vector3(mbPos.x + mbWidth - mvWidth, 0, mbPos.z - mvDepth);

    // Master Bath: South of Vestibule.
    // Vestibule South wall is at Z = mvPos.z. MBath is masterBathDepth deep.
    // Align East edges: (mvPos.x + mvWidth) - masterBathWidth
    ...
    // Centered along MB's depth (mbDepth). Closet itself is mcDepth long (N-S).
    ...
    // Align its North wall with Nook's North wall.
    ...
    plan.rooms.Add(new RoomData { /* MasterCloset */
        ...
        position = mcPos, atticHatchLocalPosition = new Vector3(mcWidth/2f, plan.storyHeight, mcDepth/2f), // Centered in closet ceiling
        walls = new List<WallSegment> {
    ...
    plan.rooms.Add(new RoomData { /* MasterVestibule */
        ...
        connectedRoomIds = new List<string> { "MasterBedroom", "MasterBath", "FamilyRoom", "CentralHallway" }, // Added CentralHallway
    ...
    plan.rooms.Add(new RoomData { /* DiningRoom */
        ...
        walls = new List<WallSegment> {
            ...
            // West Wall (X=0): Open railing to stairs. Minimal actual wall.
    ...
    plan.rooms.Add(new RoomData { /* Kitchen */
        ...
        walls = new List<WallSegment> {
            ...
            new WallSegment { startPoint = new Vector3(0,0,kDepth- (kDepth - 3f) ), endPoint = new Vector3(0,0,kDepth), isExterior = false }, // W (N part of Peninsula return, assumes 3ft peninsula depth)
    ...
    plan.rooms.Add(new RoomData { /* FamilyRoom */
        ...
        walls = new List<WallSegment> {
            ...
            // East Wall (X=frWidth): Part exterior (door to CP), part open to Kitchen/Nook.
            // Assume door to CP is 2.5ft wide, at the North end of this East wall (Z = frDepth-2.5 to frDepth).
    ...
    RoomData centralHallwayRoom = new RoomData {
        ...
        position = hallPos,
        walls = new List<WallSegment>(), // Populate based on detailed connections
    ...
    // Example: Hall's West wall segment for closet door. Closet S of HallBath, door faces East into Hall.
    // Hall X runs from hallPos.x to hallPos.x + hallWidth. West wall is at hallPos.x.
    // If HallBath is North of Office, and Office is Z=[20,30] from world origin, HallBath Z=[30,39].
    // Closet is South of HallBath. Assume 2ft wide door.
    // If Closet is Z=[28,30] on Hall's West Wall (local X=0 for Hall), door from Z=28 to Z=30.
    // This needs careful Z alignment from global layout.
    // For now, adding a conceptual small segment.
    // centralHallwayRoom.walls.Add(new WallSegment { startPoint = new Vector3(0, 0, 10f), endPoint = new Vector3(0, 0, 12f), isExterior = false }); // Example for part of West wall
    // centralHallwayRoom.walls.Add(new WallSegment { startPoint = new Vector3(hallWidth, 0, 0f), endPoint = new Vector3(hallWidth, 0, 12f), isExterior = false }); // Example for part of East wall against stairs
    ...
    RoomData staircaseRoomData = new RoomData {
        ...
        position = stairPos,
        walls = new List<WallSegment>(), // RoomBuilder handles "StairwellEnclosure"
    ...
    // Add a WindowSpec for the Living Room test
    // Find the Living Room south wall. It's made of segments.
    // LR South Wall (Z=0): Segments are (0,0,0)-(2.3335,0,0) and (10.3335,0,0)-(lrWidth,0,0)
    // The bay window is more complex. Let's place a simple window on one of these solid segments.
    // e.g., on the segment from X=0 to X=2.3335 (local to LivingRoom)
    // The wallId would be "Wall_LivingRoom_S_part1" (hypothetical name, depends on WallBuilder naming)
    // For now, let's assume a generic wallId that WindowPlacer might find or you update.
    // We need to ensure WallBuilder assigns discoverable names or WallPlacer uses different logic.
    // For this example, let's create a conceptual wallId.
    // The actual wallId needs to match a GameObject name generated by WallBuilder.
    // For now, the user will have to manually find a generated wall segment name.
    plan.windows.Add(new WindowSpec
    {
        windowId = "LivingRoom_SouthTestWindow",
        type = WindowType.SingleHung, // Make sure "Window_SingleHung_Placeholder.prefab" exists in Resources
        width = 3f,
        height = 4f,
        position = new Vector3(1f, 0f, 0f), // X: 1ft along the chosen wall segment, Y: will be set by sillHeight, Z: flush
        sillHeight = 3f, // 3ft from the floor
        wallId = "Wall_LivingRoom_7", // EXAMPLE - REPLACE with actual generated wall name
        isOperable = true
    });

    // Add other placeholder DoorSpecs, OpeningSpecs as you had them
...
// CreateHousePlanMenuItem method remains the same
...
private static void CreateHousePlanMenuItem()
{
    ...
    {
        // We need just the filename for CreateHousePlanAsset, not the full path relative to Assets/
